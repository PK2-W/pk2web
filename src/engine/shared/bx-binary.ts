import { int, uint8, uint32, uint16, bit, uint } from './bx-ctypes';
...
export class Binary {
    public static ErrorClass = Error;
    ...
    public static fromArrayBuffer(ab: ArrayBuffer): Binary {
        return new Binary(ab.slice(0));
    }
    
    public constructor(initSize: int);
    public constructor(ab: ArrayBuffer);
    public constructor(k: int | ArrayBuffer) {
        this.buffer = (k instanceof ArrayBuffer) ? k : new ArrayBuffer(k);
    ...
    public clone(): Binary {
        return new Binary(this.buffer.slice(0));
    }
    ...
    public readUint1(byteOffset: number, bitOffset: number): uint8 {
        if (bitOffset > 7) {
            throw new Binary.ErrorClass('Bit offset out of byte.');
        }
    ...
    /**
     * @param byteOffset
     * @param bitOffset - From 0 to 7 -> from right to left
     * @param value
     */
    public writeBit(byteOffset: number, bitOffset: number, value: bit): void {
        ...
        this.writeUint8(byteOffset, byte);
    }
    ...
    public getUint8Array(): Uint8Array {
        if (this._xUint8Array == null) {
            this._xUint8Array = new Uint8Array(this.buffer);
        }
    ...
    public getBlob(): Blob {
        if (this._xBlob == null) {
            this._xBlob = new Blob([this.buffer]);
        }
        return this._xBlob;
    }
    ...
    public get streamOffset(): number {
        return this._streamOffset;
    }
    public set streamOffset(v: number) {
        this._streamOffset = v;
    }
    ...
    public streamWriteUint16(v: uint16): this {
        this._view.setUint16(this._streamOffset, v, true);
        this._streamOffset += 2;
        return this;
    }
    ...
    public streamWriteUint32(v: uint32): this {
        this._view.setUint32(this._streamOffset, v, true);
        this._streamOffset += 4;
        return this;
    }
    ...
    public toBase64() {
        ...
        for (let i = 0; i < len; i++) {
            pre += String.fromCharCode(bytes[i]);
        }
        return window.btoa(pre);
    }
