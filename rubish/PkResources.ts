            ...
            req.onreadystatechange = (aEvt) => {
                if (req.readyState === 4) {
                    if (req.status === 200) {
                        ...
                        return reject(new AssetNotFoundError(this._root, uri));
                    } else {
                        return reject(new AssetFetchError(this._root, uri, 'status: ' + req.status));
                    }
                }
            };
            req.onerror = () => {
                return reject(new AssetFetchError(this._root, uri, 'unknown error'));
            };
            req.send();
        });
    }
